...
def one_point_crossover(net1, net2, p):
    ...
    # delete the weights in each net and recreate them from scratch
    net1.weights = list();
    net2.weights = list();
    # assign the weights that we saved
    ...
    # make the shapes of the weights coincide (just at the cut points, we select to resize the new net accoring to the minimum size of the two nets)
    if net1.weights[cut_point[0]-1].shape[1] > net1.weights[cut_point[0]].shape[0]:
        ...
        print("1", [w.shape for w in net1.weights]);
        ...
        print("2", [w.shape for w in net1.weights]);
    else:
        pass;
    if net2.weights[cut_point[1]-1].shape[1] > net2.weights[cut_point[1]].shape[0]:
        ...
        print("3", [w.shape for w in net2.weights]);
        ...
        print("4", [w.shape for w in net2.weights]);
    else:
        pass;    
    ...
    if (net1.fully_connected is False) and (net2.fully_connected is False): 
        ...
        # delete the masks in each net and recreate them from scratch
        net1.mask = list();
        net2.mask = list();
