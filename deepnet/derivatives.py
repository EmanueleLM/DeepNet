...
In this module we calculate the first part of each derivative
     (something like the graphical caluclation)
For example if we want to derive sigma(z) = 1/(1+(exp(-z))), we obtain 
    sigma(z)(1-sigma(z))*d(z), but we skip 
...
# please note that this one holds it returns a numpy array of size (2,1) 
#   since we have two different derivatives for each part
#   (real, imaginary) of the loss, and each of them must be multiplied to 
#   different factors according the the chain rule 
def dy_L2(y, t):
...
# which is basically the signum of the difference between t and y 
#   (that's why in regression a lot of weights are zero)
def dy_L1(y, t):
    return -np.sign(t-y);
...
# we expect that the number of dimensions is the first dimension of both the 
#   input (so y.shape[1]=t.shape[1]=dimensions)
# please note can be written as entropy plus cross entropy of target/prediction
def dy_KL(y, t):
    np.clip(y, 1e-10, 1-1e-10, out=y);
    entropy = 1+np.log2(y);
...
# (partial) derivative of the function known as Variational Autoencoders Loss 
#   (VAE)
# y is the value predicted by our algorithm
# t is the real value (we are in a supervised scenario)
# we expect that the number of dimensions is the first dimension of both the 
#   input (so y.shape[1]=t.shape[1]=dimensions)
# please note that VAE loss can be written as entropy of prediction minus kl 
...
# the partial is not in the common math sense, but in the sense that 
#   the "residual" dZ is not calculated at this step
def dsigmoid(z):
