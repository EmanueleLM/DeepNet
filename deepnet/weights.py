...
Module that initializes the weights for the network wrt to different approaches 
    (random, uniform, lecun etc.)
"""

import numpy as np

# function that returns a weights' numpy array initialized to random values 
#   between 0 and 1
def random_weights(weights):
    # use broadcasting and this version of randint because it's easier to manage 
    #   the shape of individual weight's vectors
    return np.random.randint(0, 101, weights.shape)/100; 

# function that returns a weights' numpy array initialized to uniform values 
#   between upper and lower
# takes as input
#   weights, the weights to fill with uniform values
#   lower, upper, rispectively the min a and max values on whose the uniform 
#       distribution of the weights is defined
...
# If a neuron has 10 incoming synapses, it will recieve a weight from [-1/10,+1/10], 
#   uniformely distributed
# CAVEAT: please note that the weights are supposed to be submitted in such a 
#   way that the column's dimension is the number of incoming connections
...
# flashcard weights initialization (I don't know where it exactly comes from,
#    but it is taken from machinelearningflashcards <dot> com)
